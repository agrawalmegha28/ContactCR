...
public class ToDoDBAdapter {
  ...
  //Insert a new task
  public long insertTask(AndContacts _task) {
    // Create a new row of values to insert.
    ContentValues newTaskValues = new ContentValues();
    // Assign values for each row.
    newTaskValues.put(KEY_TASK, _task.getTaskId());
   // newTaskValues.put(KEY_CREATION_DATE, 
                    // _task.getCreated().getTime());
    // Insert the row.
  ...
  public boolean updateTask(long _rowIndex, String _task) {
    ...
    return db.update(DATABASE_TABLE, newValue, KEY_ID + "=" 
                     + _rowIndex, null) > 0;
  }
  ...
  public AndContacts getToDoItem(long _rowIndex) throws SQLException {
    ...
    AndContacts result = new AndContacts();
    return result;
  }
